...
import { commands, l10n as localization, window } from "vscode";
...
export function packageInstall(outputChannel: OutputChannel, command: string, cwd: string): void {
  outputChannel.clear();
  outputChannel.show();
  outputChannel.append(`${t("Installing selected packages...")}\n\n---\n\n${command}\n\n---\n`);
  ...
  process.on("close", () => {
    ...
    if (hasError) {
      void window.showErrorMessage(t("Failed to install packages. Check the output console."));
    } else {
      void window.showInformationMessage(t("Packages installed successfully!"));
    }
  });
}
